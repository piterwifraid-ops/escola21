...
export default function Chat() {
  ...
  const handleSelectOption = async (optionId: string, optionText: string) => {
    // Redirecionamento para checkout externo ao clicar em Finalizar Inscrição
  ...
  // Fase 4: Material de Estudos e Finalização
  await addTypingMessage('Agora falta apenas um passo para garantir sua vaga: a finalização da inscrição.', 300, 'warning');
  await addTypingMessage('O material de estudo oficial é obrigatório para que você esteja preparada e para validar sua participação no programa.', 200, 'trust');
  await addTypingMessage('A taxa de 58,40 reais garante seu acesso ao material e confirma sua vaga. Não deixe para depois, pois a concorrência é alta e as vagas são limitadas.', 200, 'urgency');
  await addTypingMessage('Finalize sua inscrição agora mesmo para não perder essa oportunidade única!', 200, 'benefit');
      ...
      await new Promise<void>(resolve => {
        setTimeout(() => {
          const msg: Message = {
            ...
            text: '',
            options: [
              { id: 'pix-final', text: 'Finalizar Inscrição' },
  ...
  await addTypingMessage('Por questão de segurança, confirme seu ano de nascimento para gerar sua guia de inscrição.', 200, 'trust');
    ...
    if (optionId.startsWith('ano-')) {
      ...
      setIsLoading(true);
  await addTypingMessage('Garantindo o envio do material de estudos e gerando sua guia de inscrição segura...', 300, 'success');
