...
const validateCEP = async (cep: string) => {
	try {
		// Call our serverless proxy to avoid CORS issues when deployed
		const resp = await axios.get(`/api/cep?cep=${cep}`);
		if (!resp.data) throw new Error('CEP não encontrado');

		const { address, coordinates, schools } = resp.data;

		// Map to the same shape expected by the rest of the component
		const addressData = {
		...
		// Compute distances if lat/lon available on schools
		const processedSchools = (schools || []).map((s: any) => ({
			id: s.id,
			name: s.name,
			type: s.type || 'Escola pública',
			distance: calculateDistance(coordinates.lat, coordinates.lon, s.lat || coordinates.lat, s.lon || coordinates.lon),
		}))
