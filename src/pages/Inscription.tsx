...
const validateCEP = async (cep: string) => {
	try {
		// Try ViaCEP first, fallback to BrasilAPI if ViaCEP fails
		let addressData: any = null;

		// Use BrasilAPI as primary CEP provider
		try {
			console.debug('[CEP] Requesting BrasilAPI for', cep);
			const brasilResp = await requestWithRetry(() => axios.get(`https://brasilapi.com.br/api/cep/v1/${cep}`), 3, 500);
			console.debug('[CEP] BrasilAPI response:', brasilResp && (brasilResp as any).data);
			addressData = (brasilResp as any).data;
			// normalize keys to match ViaCEP shape
			if (!addressData.logradouro && addressData.street) addressData.logradouro = addressData.street;
			if (!addressData.bairro && addressData.neighborhood) addressData.bairro = addressData.neighborhood;
			if (!addressData.localidade && addressData.city) addressData.localidade = addressData.city;
			if (!addressData.uf && addressData.state) addressData.uf = addressData.state;
		} catch (err) {
			console.error('[CEP] BrasilAPI failed for', cep, err);
			throw new Error('CEP não encontrado');
		}

		// 2. Get coordinates from Nominatim
		const address = `${addressData.logradouro || ''}, ${addressData.localidade || ''}, ${addressData.uf || ''}, Brazil`;
		console.debug('[CEP] Requesting Nominatim for', address);
		...
		console.debug('[CEP] Nominatim response length:', (nominatimResponse as any).data && (nominatimResponse as any).data.length);
		if (!((nominatimResponse as any).data && (nominatimResponse as any).data.length)) {
			console.error('[CEP] Nominatim did not return results for address:', address);
