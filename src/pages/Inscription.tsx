...
const validateCEP = async (cep: string) => {
	try {
		// 1. Get address from BrasilAPI
		const viaCepResponse = await axios.get(`https://brasilapi.com.br/api/cep/v1/${cep}`);
		if (!viaCepResponse.data) {
			throw new Error("CEP não encontrado");
		}

		// Map BrasilAPI response to ViaCEP-like shape used by the app
		const apiAddress = {
			cep: viaCepResponse.data.cep,
			logradouro: viaCepResponse.data.street || "",
			bairro: viaCepResponse.data.neighborhood || "",
			localidade: viaCepResponse.data.city || "",
			uf: viaCepResponse.data.state || "",
		};

		// 2. Get coordinates from Nominatim
		const address = `${apiAddress.logradouro}, ${apiAddress.localidade}, ${apiAddress.uf}, Brazil`;
